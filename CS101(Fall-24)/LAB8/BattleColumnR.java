...
public class BattleColumnR
{
     ...
     */
    public BattleColumnR(boolean hasDarkKnights) 
    {
        elements = new char[SkyBattleGameR.ROWS];
    ...
    private void initEmptyColumn() 
    {
        ...
        {
            elements[i] = SkyBattleGameR.EMPTY_COL_SYMBOL;
        }
    }
    ...
    {
        int totalDarkKnights = (random.nextInt(SkyBattleGameR.MAX_DARK_KNIGHT_SIZE)) + 1;
        for(int i = 0; i < totalDarkKnights; i++)
        {
            int position = random.nextInt(SkyBattleGameR.ROWS);
            elements[position] = SkyBattleGameR.DARK_KNIGHT_SYMBOL;
        }
    }

    private void addInvisibleSword(Random random) 
    {
        int isAdd = random.nextInt(2);
        if(isAdd == 1)
        {
            int position = random.nextInt(SkyBattleGameR.ROWS);
            elements[position] = SkyBattleGameR.SWORD_SYMBOL;
        }
    }
