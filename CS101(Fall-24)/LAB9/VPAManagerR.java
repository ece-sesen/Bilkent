...
public class VPAManagerR
{
    private static ArrayList<UserR> users = new ArrayList<>();
    private static ArrayList<PetR> unownedPets = new ArrayList<>();
    private static UserR currentUser;
    ...
    private static void registerUser(Scanner scanner) 
    {
        ...
        UserR newUser = new UserR(username, password); // Pass both username and password
        users.add(newUser);
    ...
    private static void adoptNewPet(Scanner scanner) {
        if(currentUser.getPetList().size() < 3)
        {
            ...
            currentUser.adoptPet(makeCapital(petName), petType);
        }
        else
        {
    ...
    private static void adoptUnownedPet(Scanner scanner) {
        if(currentUser.getPetList().size() < 3)
        {
            ...
            if(isThere(petType) > 0)
            {
                for (int i = 0; i < unownedPets.size(); i++) 
                {
                    if( unownedPets.get(i).getType().equals(petType))
                    {
                    ...
                    {
                        PetR pet = unownedPets.remove(index);
            ...
            {
                System.out.println("No unowned " + petType + " available.");
            }
        }
    }

    private static int isThere(String type )
    {
        int amount = 0;
        for (int i = 0; i < unownedPets.size(); i++) 
        {
            if( unownedPets.get(i).getType().equals(type))
            {
               amount++;
            }
        }
    ...
    private static void releasePet(Scanner scanner) {
        ...
        String petName = scanner.nextLine();
        PetR pet = currentUser.getPetByName(makeCapital(petName));
        if (pet != null) {
            PetR releasedPet = currentUser.releasePet(pet);
            if (releasedPet != null) {
    ...
    private static void playWithPet(Scanner scanner) {
        ...
        String petName = scanner.nextLine();
        PetR pet = currentUser.getPetByName(petName);
        if (pet != null) {
            pet.playWithPet();
        } else {
            System.out.println("Pet not found.");
    ...
    private static void searchPetsByHappiness(Scanner scanner) {
        ...
        scanner.nextLine(); // Consume newline
        ArrayList<PetR> pets = currentUser.searchPetsByHappiness(minHappiness);
        if (pets.isEmpty()) {
    ...
    private static void searchPetsByHealth(Scanner scanner) {
        ...
        scanner.nextLine(); // Consume newline
        ArrayList<PetR> pets = currentUser.searchPetsByHealth(minHealth);
        if (pets.isEmpty()) {
    ...
    private static String makeCapital(String name)
    {
